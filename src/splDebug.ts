 ...
 *--------------------------------------------------------*/
/*
 * splDebug.ts implements the Debug Adapter that "adapts" or translates the Debug Adapter Protocol (DAP) used by the client (e.g. VS Code)
 * into requests and events of the real "execution engine" or "debugger" (here: class SPLRuntime).
 ...
 *
 * The most important class of the Debug Adapter is the SPLDebugSession which implements many DAP requests by talking to the SPLRuntime.
 */
...
import { SPLRuntime, IRuntimeBreakpoint, FileAccessor, RuntimeVariable, timeout, IRuntimeVariableType } from './splRuntime';
import { Subject } from 'await-notify';
import * as base64 from 'base64-js';

/**
 * This interface describes the spl-debug specific launch attributes
 * (which are not part of the Debug Adapter Protocol).
 * The schema for these attributes lives in the package.json of the spl-debug extension.
...
export class SPLDebugSession extends LoggingDebugSession {

	// we don't support multiple threads, so we can use a hardcoded ID for the default thread
	private static threadID = 1;

	// a SPL runtime (or debugger)
	private _runtime: SPLRuntime;
	...
	public constructor(fileAccessor: FileAccessor) {
		...
		this._runtime = new SPLRuntime(fileAccessor);

		this._runtime.on('stopOnStep', () => {
			this.sendEvent(new StoppedEvent('step', SPLDebugSession.threadID));
		});
		this._runtime.on('stopOnBreakpoint', () => {
			this.sendEvent(new StoppedEvent('breakpoint', SPLDebugSession.threadID));
		});
		this._runtime.on('stopOnDataBreakpoint', () => {
			this.sendEvent(new StoppedEvent('data breakpoint', SPLDebugSession.threadID));
		});
		this._runtime.on('stopOnInstructionBreakpoint', () => {
			this.sendEvent(new StoppedEvent('instruction breakpoint', SPLDebugSession.threadID));
		});
		this._runtime.on('stopOnException', (exception) => {
			if (exception) {
				this.sendEvent(new StoppedEvent(`exception(${exception})`, SPLDebugSession.threadID));
			} else {
				this.sendEvent(new StoppedEvent('exception', SPLDebugSession.threadID));
			}
		});
	...
	protected threadsRequest(response: DebugProtocol.ThreadsResponse): void {
		...
		response.body = {
			threads: [
				new Thread(SPLDebugSession.threadID, "thread 1"),
				new Thread(SPLDebugSession.threadID + 1, "thread 2"),
			]
		};
		this.sendResponse(response);
	}
	...
	private createSource(filePath: string): Source {
		return new Source(basename(filePath), this.convertDebuggerPathToClient(filePath), undefined, undefined, 'spl-adapter-data');
	}
}
